...
<script>
import { queryPageList } from '@/api/platform/auth/app'
    ...
    return {
      ...
      listConfig: {
        ...
        searchForm: {
          labelWidth: 100,
          forms: [
            { prop: 'Q^app_key_^SL', label: '应用KEY', itemWidth: '200' },
            { prop: 'Q^app_name_^SL', label: '应用名', itemWidth: '200' }
          ]
        },
        // 表格字段配置
        columns: [
          { prop: 'appKey', label: '应用key' },
          { prop: 'appName', label: 'API标识' }
        ]
      },
      pagination: {},
      sorts: {}
